...
## Step 9: perform gene-level QC by filtering out rows with less than 50 reads across all the samples
dim(deseq2Data)
dim(deseq2Data[rowSums(counts(deseq2Data)) > 50, ]) # assess filter effect
deseq2Data.gqc <- deseq2Data[rowSums(counts(deseq2Data)) > 50, ] # run and output filtered data
