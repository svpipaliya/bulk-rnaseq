...
- Important! Create a log directory `mkdir logs` where the slurm error and output files will be sent. If this is not present, serial job submission will not occur and the main batch submission script will be run indefinetly (or well atleast until the max-time specified in the script).
